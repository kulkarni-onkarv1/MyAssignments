    ...
    internal class FecthingRecords
    {
        ...
        public static void GetAllEmployeesByDeptName(string dname)
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine($"Something Went Wrong!No Record Found For Employee With Department {dname}");
                }
                else
                {
        ...
        public static void GetAllEmployeesWithMaxSalByDeptName(string dname)
        {
            ...
            try
            {
                ...
                while (sqlDataReader.Read())
                {
                    /*var GetObjectType = sqlDataReader["Salary"].GetType();
                    Console.WriteLine(GetObjectType);*/

                    if (sqlDataReader["Salary"].GetType().Equals(typeof(System.DBNull)))
                    {
                        Console.WriteLine("Something Went Wrong!No Record Found For Employee With Department:{0}", dname);
                    }
                    else
                    {
        ...
        public static void GetAllEmployeesByLocation(string location)
        {
            ...
            try
            {
                ...
                if (!sqlDataReader.Read())
                {
                    Console.WriteLine($"Something Went Wrong!No Record Found For Employee With Location {location}");
                }
                while (sqlDataReader.Read())
                {
                    Console.WriteLine($"Location: {sqlDataReader["Location"]}, EmpNo: {sqlDataReader["EmpNo"]},EmpName: {sqlDataReader["EmpName"]}, Designation: {sqlDataReader["Designation"]},  DeptName: {sqlDataReader["DeptName"]}");
                }
                sqlDataReader.Close();
            }
            catch (Exception ex)
